...
export class MockV1Pair extends Pair {
  constructor(etherAmount: BigintIsh, tokenAmount: TokenAmount) {
    super(tokenAmount, new TokenAmount(WBNB[tokenAmount.token.chainId], etherAmount))
  }
}
...
function useMockV1Pair(inputCurrency?: Currency): MockV1Pair | undefined {
  ...
  const isWBNB = Boolean(token && token.equals(WBNB[token.chainId]))
  const v1PairAddress = useV1ExchangeAddress(isWBNB ? undefined : token?.address)

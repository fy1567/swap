...
import { Currency, CurrencyAmount, Fraction, JSBI, Percent, Token, TokenAmount, WBNB } from 'swap-bsc-sdk'
    ...
    <>
      <AutoRow style={{ justifyContent: 'flex-start', width: 'fit-content' }}>
        ...
        <div style={{ marginLeft: '.75rem' }}>
          <TYPE.mediumHeader>
            ...
            {chainId && token.equals(WBNB[chainId]) ? 'WBNB' : token.symbol}/BNB
          </TYPE.mediumHeader>
        </div>
      </AutoRow>
      ...
      <RowBetween my="1rem">
        <Text fontSize={16} fontWeight={500}>
          Pooled {chainId && token.equals(WBNB[chainId]) ? 'WBNB' : token.symbol}:
        </Text>
        <RowFixed>
...
function V1PairMigration({ liquidityTokenAmount, token }: { liquidityTokenAmount: TokenAmount; token: Token }) {
  ...
  const [v2PairState, v2Pair] = usePair(chainId ? WBNB[chainId] : undefined, token)
  const isFirstLiquidityProvider: boolean = v2PairState === PairState.NOT_EXISTS

  const v2SpotPrice = chainId && v2Pair ? v2Pair.reserveOf(token).divide(v2Pair.reserveOf(WBNB[chainId])) : undefined
  ...
  return (
    <AutoColumn gap="20px">
      <TYPE.body my={9} style={{ fontWeight: 400 }}>
        ...
        {chainId && (
          <ExternalLink href={getBscScanLink(chainId, MIGRATOR_ADDRESS, 'address')}>
            <TYPE.blue display="inline">OMGSwap migration contract↗</TYPE.blue>
          </ExternalLink>
        )}
        .
      </TYPE.body>
      ...
      <TYPE.darkGray style={{ textAlign: 'center' }}>
        {`Your OMGSwap V1 ${token.symbol}/BNB liquidity will become OMGSwap V2 ${token.symbol}/BNB liquidity.`}
      </TYPE.darkGray>
    </AutoColumn>
  )
}
  ...
  return (
    <BodyWrapper style={{ padding: 24 }}>
      <AutoColumn gap="16px">
        ...
        ) : validatedAddress && chainId && token?.equals(WBNB[chainId]) ? (
          <>
            <TYPE.body my={9} style={{ fontWeight: 400 }}>
              Because OMGSwap V2 uses WBNB under the hood, your OMGSwap V1 WBNB/BNB liquidity cannot be migrated. You

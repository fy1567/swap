...
describe('v1SwapArguments', () => {
  const USDC_WBNB = new MockV1Pair('1000000', new TokenAmount(BUSD, '1000000'))
  const DAI_WBNB = new MockV1Pair('1000000', new TokenAmount(DAI, '1000000'))

  // just some random address
  const TEST_RECIPIENT_ADDRESS = USDC_WBNB.liquidityToken.address
  ...
  it('exact eth to token', () => {
    const trade = Trade.exactIn(new Route([USDC_WBNB], ETHER), CurrencyAmount.ether('100'))
  ...
  it('exact token to eth', () => {
    const trade = Trade.exactIn(new Route([USDC_WBNB], BUSD, ETHER), new TokenAmount(BUSD, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
  ...
  it('exact token to token', () => {
    const trade = Trade.exactIn(new Route([USDC_WBNB, DAI_WBNB], BUSD), new TokenAmount(BUSD, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
  ...
  it('eth to exact token', () => {
    const trade = Trade.exactOut(new Route([USDC_WBNB], ETHER), new TokenAmount(BUSD, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
  ...
  it('token to exact eth', () => {
    const trade = Trade.exactOut(new Route([USDC_WBNB], BUSD, ETHER), CurrencyAmount.ether('100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
  ...
  it('token to exact token', () => {
    const trade = Trade.exactOut(new Route([USDC_WBNB, DAI_WBNB], BUSD), new TokenAmount(DAI, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
